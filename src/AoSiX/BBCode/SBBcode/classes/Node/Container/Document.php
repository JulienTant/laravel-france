...
class Node_Container_Document extends Node_Container
{
    ...
    public static function default_bbcodes()
    {
        return array(
            ...
            new BBCode('code', function ($content, $attribs, $node) {
                $content = '';
                foreach ($node->children() as $child)
    ...
    {
        $this->loop_text_nodes(
            function ($child) {
                preg_match_all(
                    "/(?:(?:https?|ftp):\/\/|(?:www|ftp)\.)(?:[a-zA-Z0-9\-\.]{1,255}\.[a-zA-Z]{1,20})(?::[0-9]{1,5})?(?:\/[^\s'\"]*)?(?:(?<![,\)\.])|[\S])/",
                    $child->get_text(),
                    $matches,
                ...
                foreach ($matches[0] as $match) {
                    ...
                    $url = new Node_Container_Tag('url', array(
    ...
    public function detect_emails()
    {
        $this->loop_text_nodes(
            function ($child) {
                preg_match_all(
                    "/(?:[a-zA-Z0-9\-\._]){1,}@(?:[a-zA-Z0-9\-\.]{1,255}\.[a-zA-Z]{1,20})/",
                    $child->get_text(),
                    $matches,
                    PREG_PATTERN_ORDER | PREG_OFFSET_CAPTURE
                );
    ...
    public function detect_emoticons()
    {
        ...
        $this->loop_text_nodes(
            function ($child) use ($pattern, $emoticons) {
                preg_match_all(
                    $pattern,
                    $child->get_text(),
                    $matches,
